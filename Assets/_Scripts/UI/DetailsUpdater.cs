...
namespace TheDragonHunt
{
    public class DetailsUpdater : MonoBehaviour
    {
        ...
        [SerializeField] private int _maxObjectPerRow = 5;
        ...
        private void OnEnable()
        {
            MessageQueueManager.Instance.
                AddListener<UpdateDetailsMessage>(OnDetailsUpdated);
        }
        private void OnDisable()
        {
            MessageQueueManager.Instance.RemoveListener<UpdateDetailsMessage>(
                OnDetailsUpdated);
        }
        ...
        private void UpdateRows(List<UnitComponent> units)
        {
            ...
            {
                if(i < _maxObjectPerRow)
                {
                    AddObjectToRow(_row1, _objectPool.GetObject());
                }
                else if (i < _maxObjectPerRow * 2)
                {
                ...
                {
                    Debug.Log(
                        $"More than {_maxObjectPerRow * 2} units selected");
                    break;
                }
            }
        }
        ...
        private void UpdateModel(GameObject model)
        {
            ...
            newPortrait.SetLayerMaskAllChildren("Portrait");
        }
        private void ResetTransform(GameObject portrait)
        {
            ...
            portrait.transform.SetParent(
                _portraitModel.transform, false);
        }
        private void RemoveUnitComponent(GameObject portrait)
        {

            UnitComponent unit =
                portrait.GetComponent<UnitComponent>();
            unit.Selected(false);
            Destroy(unit);
            ...
            BoxCollider boxCollider =
                portrait.GetComponent<BoxCollider>();
            Destroy(boxCollider);
        }
    }
}
